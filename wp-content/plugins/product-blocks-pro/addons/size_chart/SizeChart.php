...
class SizeChart {
    ...
    public function custom_post_register() {
        $labels = array(
            'name'                  => esc_html_x( 'Size Chart', 'Post Type General Name', 'product-blocks-pro' ),
            'singular_name'         => esc_html_x( 'Size Chart', 'Post Type Singular Name', 'product-blocks-pro' ),
            'menu_name'             => esc_html__( 'Size Chart', 'product-blocks-pro' ),
            'name_admin_bar'        => esc_html__( 'Size Chart', 'product-blocks-pro' ),
            'archives'              => esc_html__( 'Size Chart Archives', 'product-blocks-pro' ),
            'all_items'             => esc_html__( 'Size Charts', 'product-blocks-pro' ),
            'add_new'               => esc_html__( 'Add New Chart', 'product-blocks-pro' ),
            'add_new_item'          => esc_html__( 'Add New Chart', 'product-blocks-pro' ),
            'edit_item'             => esc_html__( 'Edit Chart', 'product-blocks-pro' ),
            'update_item'           => esc_html__( 'Update Chart', 'product-blocks-pro' ),
            'view_item'             => esc_html__( 'View Chart', 'product-blocks-pro' ),
            'search_items'          => esc_html__( 'Search Size Charts', 'product-blocks-pro' ),
            'not_found'             => esc_html__( 'Not found', 'product-blocks-pro' ),
            'not_found_in_trash'    => esc_html__( 'Not found in Trash', 'product-blocks-pro' ),
            'featured_image'        => esc_html__( 'Size chart Image', 'product-blocks-pro' ),
            'set_featured_image'    => esc_html__( 'Set size chart image', 'product-blocks-pro' ),
            'remove_featured_image' => esc_html__(  'Remove image', 'product-blocks-pro' ),
            'use_featured_image'    => esc_html__( 'Use as size chart image', 'product-blocks-pro' ),
            'insert_into_item'      => esc_html__( 'Insert into Size Chart', 'product-blocks-pro' ),
            'uploaded_to_this_item' => esc_html__( 'Uploaded to this Size Chart', 'product-blocks-pro' ),
            'items_list'            => esc_html__( 'Size Charts', 'product-blocks-pro' ),
            'items_list_navigation' => esc_html__( 'Size Charts navigation', 'product-blocks-pro' ),
            'filter_items_list'     => esc_html__( 'Filter from charts', 'product-blocks-pro' ),
        );
        $chart_arg = array(
            'label'               => esc_html__( 'Size Chart', 'product-blocks-pro' ),
            'description'         => esc_html__( 'Size Chart', 'product-blocks-pro' ),
            'labels'              => $labels,
    ...
    public function size_chart_meta_boxes() {
        add_meta_box(
            ...
            __( 'Chart Table', 'product-blocks-pro' ),
        ...
        add_meta_box(
            ...
            __( 'Assigning Chart', 'product-blocks-pro' ),
    ...
    public function table_field_callback() {
        ...
        $head_default       = __( 'Title', 'product-blocks-pro' );
        $content_default    = __( 'Content', 'product-blocks-pro' );
    ...
    ?>
        <p class="wopb-toogle-field-wrap">
            <span class="wopb-label">
                <?php echo esc_html__('Table Heading Position', 'product-blocks-pro'); ?>
            </span>
            <span class="wopb-toggle-control">
                <span><?php echo esc_html__('Column', 'product-blocks-pro'); ?></span>
                ...
                <span><?php echo esc_html__( 'Row', 'product-blocks-pro' ); ?></span>
            </span>
        </p>
        <p class="wopb-toogle-field-wrap">
            <span class="wopb-label">
                <?php echo esc_html__('Hide Chart Name', 'product-blocks-pro'); ?>
            </span>
            <span class="wopb-toggle-control">
                <span>
                    <input
        ...
        <table class="wopb-sc-table">
            <tbody>
                <tr class="wopb-sc-control-col">
                    <?php  if ( isset( $chart_columns[0] ) ) {
                        foreach ( $chart_columns[0] as $row_index => $column ) { ?>
                            <td>
                                <a class="wopb-sc-control-btn wopb-sc-add-item">
                                    <span class="dashicons dashicons-plus-alt2"></span>
                                    <span class="wopb-sc-tooltip"><?php echo esc_html__( 'Add Column', 'product-blocks-pro' ); ?></span>
                                </a>
                                <a class="wopb-sc-control-btn wopb-sc-del-item">
                                    <span class="dashicons dashicons-minus"></span>
                                    <span class="wopb-sc-tooltip"><?php echo esc_html__( 'Remove Column', 'product-blocks-pro' ); ?></span>
                                </a>
                            </td>
                        <?php }
                    } ?>
                </tr>
                <?php if ( is_array( $chart_columns ) ) {
                            ...
                            <td class="wopb-sc-control-row">
                                <?php if ( $row_index > 0 ) { ?>
                                    <a class="wopb-sc-control-btn wopb-sc-add-item<?php echo esc_attr( $row_control_class ); ?>">
                                        <span class="dashicons dashicons-plus-alt2"></span>
                                        <span class="wopb-sc-tooltip"><?php echo esc_html__( 'Add Row', 'product-blocks-pro' ); ?></span>
                                    </a>
                                    <a class="wopb-sc-control-btn wopb-sc-del-item<?php echo esc_html( $row_control_class ); ?>">
                                        <span class="dashicons dashicons-minus"></span>
                                        <span class="wopb-sc-tooltip"><?php echo esc_html__( 'Remove Row', 'product-blocks-pro' ); ?></span>
                                    </a>
                                <?php } ?>
                            </td>
                        </tr>
    ...
    public function assign_fields_callback() {
        ...
        <p>
            ...
            <label for="wopb-sc-all-product">
                <?php echo esc_html__( 'Apply On All Products ?', 'product-blocks-pro' ); ?>
            </label>
        </p>
        <div class="wopb-sc-category<?php echo $all_product_selected ? ' wopb-d-none' : ''; ?>">
            <label><?php echo esc_html__( 'Assign Category', 'product-blocks-pro' ); ?></label>
            <div>
        ...
        ! empty( $include_products ) ? $include_products : [] ,
            __( 'Include Products', 'product-blocks-pro' ),
            'wopb-sc-include-products' . ( $all_product_selected ? ' wopb-d-none' : '' )
        );
        $this->render_product_select_field(
        'wopb_sc_exclude_products',
        ! empty( $exclude_products ) ? $exclude_products: [],
            __( 'Exclude Products', 'product-blocks-pro' ),
     ...
     * @since v.3.2.0
     */
    private function render_product_select_field( string $key, array $selected_products, string $label, string $class = '' ) {
    ?>
        <p class="wopb-sc-products <?php echo esc_attr( $class ); ?>">
            <label for="<?php echo esc_attr( $key ); ?>">
                <?php
                    echo esc_html( $label ) . wc_help_tip( __( 'Select Products List To Show Specific Product.', 'product-blocks-pro' ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                ?>
            </label>
            <select
                ...
                multiple="multiple"
                data-placeholder="<?php echo esc_attr__( 'Search for a product', 'product-blocks-pro' ); ?>"
    ...
    public function size_chart_admin_product_tab( $product_data_tabs ) {
        $product_data_tabs['wopb_size_chart'] = array(
            'label'     => __( 'Size Chart', 'product-blocks-pro' ),
            ...
            <div class="wopb-field-group">
                <?php if ( $charts->have_posts() ) { ?>
                    <label for="wopb_size_chart_ids">
                        <?php echo esc_html__( 'Select Size Chart', 'product-blocks-pro' ); ?>
                    </label>
                    <select
                        ...
                        >
                        <option value=""><?php echo esc_html__( 'Select Size Chart', 'product-blocks-pro' ); ?></option>
                        <?php
                ...
                <?php } else { ?>
                    <div class="wopb-chart-create-desc">
                        <?php echo esc_html__( 'There is no chart available', 'product-blocks-pro' ); ?>
                        <a
                            href="<?php echo esc_url( admin_url( 'post-new.php?post_type=wopb-size-chart' ) ); ?>"
                            class="wopb-add-chart-btn"
                        >
                            <?php echo esc_html__( 'Create Chart', 'product-blocks-pro' ); ?>
                        </a>
                    </div>
                <?php } ?>
            </div>
