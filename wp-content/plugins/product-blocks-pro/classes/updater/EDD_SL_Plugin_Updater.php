...
class EDD_SL_Plugin_Updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
					__('There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'product-blocks-pro'),
				...
				printf(
					__('There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'product-blocks-pro'),
					esc_html( $version_info->name ),
					'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $version_info->new_version ),
					'</a>',
	...
	public function show_changelog() {
		...
		if( ! current_user_can( 'update_plugins' ) ) {
			wp_die( __('You do not have permission to install plugin updates', 'product-blocks-pro'), __('Error', 'product-blocks-pro'), array( 'response' => 403 ) );
		}
	...
	public function set_version_info_cache( $value = '', $cache_key = '' ) {
		...
		if ( ! isset( $value->tested ) ) {
			global $wp_version;
			$value->tested = $wp_version;
		}
		if ( ! isset( $value->requires_php ) ) {
			$value->requires_php = phpversion();
		}
