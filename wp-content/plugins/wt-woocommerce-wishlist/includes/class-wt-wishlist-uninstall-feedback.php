...
if (!class_exists('WT_Wishlist_Uninstall_Feedback')) :
    ...
    class WT_Wishlist_Uninstall_Feedback {
            ...
            <div class="wtwishlist-modal" id="wtwishlist-wtwishlist-modal">
                <div class="wtwishlist-modal-wrap">
                    <div class="wtwishlist-modal-header">
                        <h3><?php esc_html_e('If you have a moment, please let us know why you are deactivating:', 'wt-woocommerce-wishlist'); ?></h3>
                    </div>
                    <div class="wtwishlist-modal-body">
                        <ul class="reasons">
                            <?php foreach ($reasons as $reason) { ?>
                                <li data-type="<?php echo esc_attr($reason['type']); ?>" data-placeholder="<?php echo esc_attr($reason['placeholder']); ?>">
                                    <label><input type="radio" name="selected-reason" value="<?php echo esc_attr( $reason['id'] ); ?>"> <?php echo esc_html__( $reason['text'] ); ?></label>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
                    <div class="wtwishlist-modal-footer">
                        <a href="#" class="dont-bother-me"><?php esc_html_e('I rather wouldn\'t say', 'wt-woocommerce-wishlist'); ?></a>
                        <button class="button-primary wtwishlist-model-submit"><?php esc_html_e('Submit & Deactivate', 'wt-woocommerce-wishlist'); ?></button>
                        <button class="button-secondary wtwishlist-model-cancel"><?php esc_html_e('Cancel', 'wt-woocommerce-wishlist'); ?></button>
                    </div>
                </div>
            </div>
        ...
        public function send_uninstall_reason() {
            ...
            $resp = wp_remote_post('https://feedback.webtoffee.com/wp-json/wtwishlist/v1/uninstall', array(
                'method' => 'POST',
                'timeout' => 45,
                'redirection' => 5,
                'httpversion' => '1.0',
