...
class WT_Wishlist_Looppage {
    ...
    public function render_webtoffee_wishlist_button() {
        ...
        // Add display:none to allowed styles
        ...
        if(isset($wt_wishlist_button_style_settings_options['wt_button_type'])){
            ...
            if($wt_wishlist_button_style_settings_options['wt_button_type'] == 'text'){
                ...
                echo "<div class='".esc_attr( $icon_class )."'> <a href='#' > <img class='wishlist_text_icon_image " . esc_attr( $class ) . " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "' data-action='" . esc_attr( $action ) . "' style='margin-bottom: -2px !important;' src='".esc_url( $text_icon_src )."'><span class='" . esc_attr( $class ) . " wt-wishlist-button' data-act='add' data-action='" . esc_attr( $action ) . "' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".esc_html__( $text_title )."</span></a> ".wp_kses( $element, $allowed_html )." </div>";

            }else if($wt_wishlist_button_style_settings_options['wt_button_type'] == 'icon'){

                if($icon_position == 1){
                    echo "<div class='".esc_attr( $icon_class )."'> <a href='#' > <i class='" . esc_attr( $class ) . " wt-wishlist-button' data-act='add' data-action='" . esc_attr( $action ) . "' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'><img style='width:auto; display:inline-flex; margin-bottom: -2px !important;'src='".esc_url( $text_icon_src )."'></i> </a> ".wp_kses_post( $element )." </div>";
                }else{
                    echo "<div class='".esc_attr( $icon_class )."'> <a href='#' > <i class='" . esc_attr( $class ) . " wt-wishlist-button' data-act='add' data-action='" . esc_attr( $action ) . "' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'><img style='width:auto; display:inline-flex; margin-bottom: -2px !important;'src='".esc_url( $icon_src )."'></i> </a> ".wp_kses_post( $element )." </div>";
                }
                ...
                if($product_added == 1){
                    echo "<div class='".esc_attr( $icon_class )."'> <a href='#' > <img class='wishlist_text_icon_image " . esc_attr( $class ) . " wt-wishlist-button' data-act='add' type-action='btn' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "' data-action='" . esc_attr( $action ) . "' style='margin-bottom: -2px !important;height:16px;' src='".esc_url( $text_icon_src )."'> <span class='" . esc_attr( $class ) . " wt-wishlist-button' data-act='add' type-action='btn' data-action='" . esc_attr( $action ) . "' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".esc_html__( $button_msg )."</span></a> <button  style='margin-top:0px !important;display:none' class='button wt-wishlist-btn-removed" . esc_attr( $class ) . esc_attr( $button_class ). " wt-wishlist-button' data-act='add' data-action='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".esc_html__( $button_msg_old )."</button>". wp_kses_post( $element )." </div>";
                }else{
                    echo "<div class='".esc_attr( $icon_class )." '> <a href='#' class='wt-wishlist-btn-added' style='display:none'> <img class='wishlist_text_icon_image " . esc_attr( $class ) . " wt-wishlist-button' data-act='add' type-action='btn' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "' data-action='remove' style='margin-bottom: -2px !important;height:16px;' src='".esc_url( $text_icon_src_for_btn )."'> <span class='" . esc_attr( $class ) . " wt-wishlist-button' data-act='add' data-action='remove' type-action='btn' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".esc_html__( $text_title_for_btn )."</span></a> <button  style='margin-top:0px !important' class='button wt-wishlist-btn-removed" . esc_attr( $class ) . esc_attr( $button_class ). " wt-wishlist-button' data-act='add' data-action='" . esc_attr( $action ) . "' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".esc_html__( $button_msg )."</button>".wp_kses_post( $element )." </div>";
                }
            }
        }
