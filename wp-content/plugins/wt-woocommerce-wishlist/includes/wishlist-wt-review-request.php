 ...
 *  
 *
 * @package  Migration_Duplicator  
 */
if (!defined('ABSPATH')) {
    exit;
}
class WT_Wishlist_Review_Request
{
    /**
     * config options 
     */
    private $plugin_title               =   "Wishlist for WooCommerce";
    private $review_url                 =   "https://wordpress.org/support/plugin/wt-woocommerce-wishlist/reviews/#new-post";  
    private $plugin_prefix              =   "wtwishlist"; /* must be unique name */
    private $activation_hook            =   "wtwishlist_activate"; /* hook for activation, to store activated date */
    private $deactivation_hook          =   "wtwishlist_deactivate"; /* hook for deactivation, to delete activated date */
    private $days_to_show_banner        =   4; /* when did the banner to show */
    private $remind_days                =   2; /* remind interval in days */
    private $webtoffee_logo_url         =   '/admin/img/webtoffee-logo_small.png';
    ...
    public function __construct()
    {
        ...
        if ($this->check_condition()) /* checks the banner is active now */ {
            $this->banner_message = sprintf(__("Hey, we at %sWebToffee%s would like to thank you for using our plugin. We would really appreciate if you could take a moment to drop a quick review that will inspire us to keep going.", 'wt-woocommerce-wishlist'), '<b>', '</b>');

            /* button texts */
            $this->later_btn_text   = __("Remind me later", 'wt-woocommerce-wishlist');
            $this->never_btn_text   = __("Not interested", 'wt-woocommerce-wishlist');
            $this->review_btn_text  = __("Review now", 'wt-woocommerce-wishlist');
...
?>
        <div class="<?php echo esc_attr( $this->banner_css_class ); ?> notice-info notice is-dismissible">
            ...
            ?>
                <h3 style="margin: 10px 0;"><?php echo esc_html__( $this->plugin_title ); ?></h3>
            <?php
            }
            ?>
            <p>
            ...
            <p>
                <a class="button button-secondary" style="color:#333; border-color:#ccc; background:#efefef;" data-type="later"><?php echo esc_html__( $this->later_btn_text ); ?></a>
                <a class="button button-primary" data-type="review"><?php echo esc_html__( $this->review_btn_text ); ?></a>
            </p>
            <div class="wt-cli-review-footer" style="position: relative;">
                <span class="wt-cli-footer-icon" style="position: absolute;right: 0;bottom: 10px;"><img src="<?php echo esc_url( plugins_url(basename(plugin_dir_path(WEBTOFFEE_WISHLIST_FILENAME))).$this->webtoffee_logo_url ); ?>" style="max-width:100px;"></span>
            </div>
        </div>
    <?php
    }
...
new WT_Wishlist_Review_Request();
