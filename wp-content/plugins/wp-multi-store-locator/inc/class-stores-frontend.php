	...
	class WPMSL_Stores_Frontend{
		...
		public function store_locator_show_func($atts) {
		    ...
		    <script>
		        ...
		        setTimeout(function() {
		            wpmsl_update_map('<?php echo esc_attr($address) ?>','<?php echo esc_attr($atts['radius'])?>');
		            jQuery('#store_locatore_search_input').val('<?php echo esc_attr($address)?>');
		            jQuery('#store_locatore_search_radius option[value="<?php echo esc_attr($atts['radius'])?>"]').prop('selected', true);
		...
		public function make_search_request() {
		    ...
		    $radius       = (floatval(isset($_POST["store_locatore_search_radius"])))?("HAVING distance < ". floatval($_POST["store_locatore_search_radius"]) ):"";
			$radius = esc_sql($radius);
