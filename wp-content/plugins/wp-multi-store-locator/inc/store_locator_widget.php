...
class wp_store_locator_nearby_widget extends WP_Widget {
                            ...
                            if (results[0]) {
                                ...
                                geocoder.geocode({
                                    ...
                                    if (status == google.maps.GeocoderStatus.OK) {
                                        jQuery.ajax({
                                            url: ajax_url,
                                            data: 'action=get_nearby_stores_ajx' + '&lat=' + results[0].geometry.location.lat() + '&lng=' + results[0].geometry.location.lng() + "&limit=" + <?php echo esc_js($limit); ?> + "&distance=" + <?php echo esc_js($distance); ?>,
                                            type: 'post',
    ...
    public function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','store_locator'); ?></label> 
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
            <label for="<?php echo esc_attr($this->get_field_id('number_of_stores')); ?>"><?php esc_html_e('Number of displayed Stores:','store_locator'); ?></label> 
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('number_of_stores')); ?>" name="<?php echo esc_attr($this->get_field_name('number_of_stores')); ?>" type="text" value="<?php echo esc_attr($number_of_stores); ?>" />
            <label for="<?php echo esc_attr($this->get_field_id('distance')); ?>"><?php esc_html_e('Distance Within(miles):'); ?></label> 
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('distance')); ?>" name="<?php echo esc_attr($this->get_field_name('distance')); ?>" type="text" value="<?php echo esc_attr($distance); ?>" />
        </p>
        <?php
    }
    ...
    if ($user_lat && $user_lng) {
        ...
        if ($stores) {
            ...
            foreach ($stores as $store) {
                echo "<li>" . ' <span class="dashicons dashicons-location"></span>' . esc_html($store->post_title) . " <br><span>" . esc_html(number_format($store->distance, 2)) . "Mile. </span>" . "</li>";
            }
            echo '</ul>';
        } else {
            echo "<div class='store_locator_widget_nodata'>" . esc_html__("No Stores found near you.", "store_locator") . "</div>";
        }
    } else {
        echo "<div class='store_locator_widget_nodata'>" . esc_html__("Couldn't detect user location.", "store_locator") . "</div>";
    }
    wp_die();
}
