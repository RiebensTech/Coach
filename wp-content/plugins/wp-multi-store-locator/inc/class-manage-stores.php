...
?>
				...
				<tbody>
					<tr>
						...
						<td><?php $meta = get_post_meta($post_id, 'store_locator_code', true); ?>
							<input type="text" value="<?php echo !empty($meta) ? esc_attr($meta) : esc_attr($post_id); ?>" name="store_locator_code" class="widefat" readonly />
						</td>
					</tr>
					<tr>
						...
						<td>
							<table id="store_locator_hours" style="background-color: rgb(241, 241, 241); border-radius: 5px;" class="widefat">
								<?php foreach ($days as $day): ?>
									<tr>
										<td style="border-bottom: 1px solid #dbdbdb;"><?php echo esc_html($day); ?></td>
										<td style="border-bottom: 1px solid #dbdbdb;">
											<input <?php echo (isset($days_meta[$day]) && $days_meta[$day]['status'] == '1') ? 'checked' : ''; ?> type="radio" value="1" id="store_locator_days_<?php echo esc_attr($day); ?>_1" name="store_locator_days[<?php echo esc_attr($day); ?>][status]"> <label for="store_locator_days_<?php echo esc_attr($day); ?>_1"> Opened </label>
											<input <?php echo (!isset($days_meta[$day]) || $days_meta[$day]['status'] == '0') ? 'checked' : ''; ?> type="radio" value="0" id="store_locator_days_<?php echo esc_attr($day); ?>_0" name="store_locator_days[<?php echo esc_attr($day); ?>][status]" /> <label for="store_locator_days_<?php echo esc_attr($day); ?>_0"> Closed </label>
										</td>
										<td style="border-bottom: 1px solid #dbdbdb;">
											<input <?php echo (isset($days_meta[$day]) && $days_meta[$day]['status'] == '1') ? '' : 'style="display: none;"'; ?> size="9" placeholder="Open Time" type="text" value="<?php echo (isset($days_meta[$day])) ? esc_attr($days_meta[$day]['start']) : ''; ?>" name="store_locator_days[<?php echo esc_attr($day); ?>][start]" class="start_time" />
											<input <?php echo (isset($days_meta[$day]) && $days_meta[$day]['status'] == '1') ? '' : 'style="display: none;"'; ?> size="9" placeholder="Close Time" type="text" value="<?php echo (isset($days_meta[$day])) ? esc_attr($days_meta[$day]['end']) : ''; ?>" name="store_locator_days[<?php echo esc_attr($day); ?>][end]" class="end_time" />
										</td>
									</tr>
								<?php endforeach; ?>
							</table>
						...
						<td>
							<select name="store_locator_sales[]" id="store_locator_sales" multiple="multiple" class="widefat">
								...
								foreach ($allSales as $manager) { ?>
									<option value="<?php echo esc_attr($manager->ID); ?>" <?php echo (in_array($manager->ID, $selectedSales)) ? "selected" : ""; ?>><?php echo esc_html($manager->post_title); ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
						...
						<td>
							<select name="store_locator_country" id="store_locator_country" class="widefat">
								...
								foreach ($allCountries as $country) { ?>
									<option value="<?php echo esc_attr($country->name); ?>" <?php echo ($selectedCountry == $country->name) ? "selected" : ""; ?>><?php echo esc_html($country->name); ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
						...
						<td>
							<select name="store_locator_state" id="store_locator_state" class="widefat">
								...
								foreach ($allStates as $state) { ?>
									<option value="<?php echo esc_attr($state->name); ?>" <?php echo ($selectedState == $state->name) ? "selected" : ""; ?>><?php echo esc_html($state->name); ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
					<tr>
						<th><?php echo esc_html__("City", 'store_locator'); $city_meta = get_post_meta($post_id, 'store_locator_city', true);?></th>
						<td>
							
							<input id="store_locator_city" type="text" value="<?php echo esc_attr($city_meta) ? esc_attr($city_meta) : ''; ?>" name="store_locator_city" class="widefat" />
						</td>
					</tr>
					<tr>
						<th><?php echo esc_html__("Postal Code", 'store_locator'); ?></th>
						<td>
							<input id="store_locator_zipcode" type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'store_locator_zipcode', true)) ? esc_attr(get_post_meta($post_id, 'store_locator_zipcode', true)) : ''; ?>" name="store_locator_zipcode" class="widefat" />
						</td>
					</tr>
				</tbody>
			</table>
			<input type="hidden" value="<?php echo esc_attr(get_post_meta($post_id, 'store_locator_lat', true)) ? esc_attr(get_post_meta($post_id, 'store_locator_lat', true)) : ''; ?>" name="store_locator_lat" id="store_locator_lat" />
			<input type="hidden" value="<?php echo esc_attr(get_post_meta($post_id, 'store_locator_lng', true)) ? esc_attr(get_post_meta($post_id, 'store_locator_lng', true)) : ''; ?>" name="store_locator_lng" id="store_locator_lng" />
			<div id="map-container" style="position: relative;">
				<div id="map_loader" style="z-index: 9;width: 100%; height: 200px;position: absolute;background-color: #fff;">
					<div class="uil-ripple-css" style="transform: scale(0.6); margin-left: auto; margin-right: auto;">
						<div></div>
						<div></div>
					</div>
				</div>
			...
			<table class="widefat" style="border: 0px;">
				<tbody>
					<tr>
						...
						<td>
							<input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'sales_manager_id', true)) ? esc_attr(get_post_meta($post_id, 'sales_manager_id', true)) : ''; ?>" name="sales_manager_id" class="regular-text" />
						</td>
					</tr>
					<tr>
						<td><?php echo esc_html__("Title", 'store_locator'); ?></td>
						<td>
							<input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'sales_manager_title', true)) ? esc_attr(get_post_meta($post_id, 'sales_manager_title', true)) : ''; ?>" name="sales_manager_title" class="regular-text" />
						</td>
					</tr>
					<tr>
						<td><?php echo esc_html__("Name", 'store_locator'); ?></td>
						<td>
							<input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'sales_manager_name', true)) ? esc_attr(get_post_meta($post_id, 'sales_manager_name', true)) : ''; ?>" name="sales_manager_name" class="regular-text" />
						</td>
					</tr>
					<tr>
						<td><?php echo esc_html__("Phone", 'store_locator'); ?></td>
						<td>
							<input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'sales_manager_phone', true)) ? esc_attr(get_post_meta($post_id, 'sales_manager_phone', true)) : ''; ?>" name="sales_manager_phone" class="regular-text" />
						</td>
					</tr>
					<tr>
						<td><?php echo esc_html__("E-Mail", 'store_locator'); ?></td>
						<td>
							<input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'sales_manager_email', true)) ? esc_attr(get_post_meta($post_id, 'sales_manager_email', true)) : ''; ?>" name="sales_manager_email" class="regular-text" />
						</td>
					</tr>
				</tbody>
			</table>
		...
		public function store_locator_save_meta_box_data($post_id)
		{
			if (isset($_POST['post_type']) && 'store_locator' == $_POST['post_type']) {
				...
				if (isset($_POST['store_locator_name'])) {
					$sanitized_name = sanitize_text_field($_POST['store_locator_name']);
					update_post_meta($post_id, 'store_locator_name', $sanitized_name);
				}
				if (isset($_POST['store_locator_address'])) {
					$sanitized_address = sanitize_text_field($_POST['store_locator_address']);
					update_post_meta($post_id, 'store_locator_address', $sanitized_address);
				}
				if (isset($_POST['store_locator_lat'])) {
					$sanitized_lat = sanitize_text_field($_POST['store_locator_lat']);
					update_post_meta($post_id, 'store_locator_lat', $sanitized_lat);
				}
				if (isset($_POST['store_locator_lng'])) {
					$sanitized_lng = sanitize_text_field($_POST['store_locator_lng']);
					update_post_meta($post_id, 'store_locator_lng', $sanitized_lng);
				}
				if (isset($_POST['store_locator_country'])) {
					$sanitized_country = sanitize_text_field($_POST['store_locator_country']);
					update_post_meta($post_id, 'store_locator_country', $sanitized_country);
				}
				if (isset($_POST['store_locator_state'])) {
					$sanitized_state = sanitize_text_field($_POST['store_locator_state']);
					update_post_meta($post_id, 'store_locator_state', $sanitized_state);
				}
				if (isset($_POST['store_locator_city'])) {
					$sanitized_city = sanitize_text_field($_POST['store_locator_city']);
					update_post_meta($post_id, 'store_locator_city', $sanitized_city);
				}
				if (isset($_POST['store_locator_phone'])) {
					$sanitized_phone = sanitize_text_field($_POST['store_locator_phone']);
					update_post_meta($post_id, 'store_locator_phone', $sanitized_phone);
				}
				if (isset($_POST['store_locator_fax'])) {
					$sanitized_fax = sanitize_text_field($_POST['store_locator_fax']);
					update_post_meta($post_id, 'store_locator_fax', $sanitized_fax);
				}
				if (isset($_POST['store_locator_email'])) {
					$sanitized_email = sanitize_text_field($_POST['store_locator_email']);
					update_post_meta($post_id, 'store_locator_email', $sanitized_email);
				}
				if (isset($_POST['store_locator_website'])) {
					$sanitized_website = sanitize_text_field($_POST['store_locator_website']);
					update_post_meta($post_id, 'store_locator_website', $sanitized_website);
				}
				if (isset($_POST['store_locator_zipcode'])) {
					$sanitized_zip = sanitize_text_field($_POST['store_locator_zipcode']);
					update_post_meta($post_id, 'store_locator_zipcode', $sanitized_zip);
				}
				if (isset($_POST['store_locator_code'])) {
					$sanitized_code = sanitize_text_field($_POST['store_locator_code']);
					update_post_meta($post_id, 'store_locator_code', $sanitized_code);
				}
				if (isset($_POST['store_locator_sales'])) {
					$sanitized_sales = sanitize_text_field($_POST['store_locator_sales']);
					update_post_meta($post_id, 'store_locator_sales', $sanitized_sales);
				}
				if (isset($_POST['store_locator_days'])) {
					$sanitized_days = sanitize_text_field($_POST['store_locator_days']);
					update_post_meta($post_id, 'store_locator_days', $sanitized_days);
				}
				if (isset($_POST['store_locator_description'])) {
					$sanitized_description = sanitize_text_field($_POST['store_locator_description']);
					update_post_meta($post_id, 'store_locator_description', $sanitized_description);
				}
		...
		public function manage_stores_columns($column, $post_id)
		{
			...
			switch ($column) {
				case 'store_address':
					...
					echo esc_html($meta['store_locator_address'][0]) . " " . esc_html($meta['store_locator_city'][0]) . " " . esc_html($meta['store_locator_state'][0]) . " " . esc_html($meta['store_locator_country'][0]) . " " . esc_html($meta['store_locator_zipcode'][0]);
					break;
				case 'store_sales':
					$sales = get_post_meta($post_id, 'store_locator_sales', true);
					if ($sales) {
						foreach ($sales as $manager) {
							echo esc_html(get_post($manager)->post_title) . "<br>";
						}
					}
					break;
				default:
		...
		public function manage_sales_manager_columns($column, $post_id)
		{
			...
			switch ($column) {
				case 'sales_title':
					echo esc_html(get_post_meta($post_id, 'sales_manager_title', true));
					break;
				case 'sales_phone':
					echo esc_html(get_post_meta($post_id, 'sales_manager_phone', true));
					break;
				case 'sales_email':
					echo esc_html(get_post_meta($post_id, 'sales_manager_email', true));
					break;
				default:
					break;
			}
			...
			<?php if (isset($_GET['post_type']) && $_GET['post_type'] == 'store_locator'): ?>
				<select style="width: 186px;" name="store_locator_manager">
					...
					foreach ($q as $manager) { ?>
						<option value="<?php echo esc_attr($manager->ID); ?>" <?php echo ($manager->ID == $currentManager) ? "selected" : ""; ?>><?php echo esc_html($manager->post_title); ?></option>
					<?php } ?>
				</select>
			<?php endif; ?>
<?php
