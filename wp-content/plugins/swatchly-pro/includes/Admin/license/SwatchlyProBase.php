<?php
if(!class_exists("SwatchlyProBase")) {
    class SwatchlyProBase {
        public $key = "2D7131277FB60C63";
        private $product_id = "12";
        private $product_base = "swatchly-pro";
        private $server_host = "https://license.hasthemes.com/wp-json/licensor/";
        ...
        {
            $this->pluginFile=SWATCHLY_PRO_FILE;
            ...
            if($this->hasCheckUpdate) {
                if(function_exists("add_action")){
                    add_action( 'admin_post_swatchly-pro_fupc', function(){
                    ...
                    if($this->isTheme){
                        ...
                        add_filter( 'plugin_row_meta', function($links, $plugin_file ){
                            if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
                                $links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=swatchly-pro_fupc' ) . "'>Update Check</a>";
                            }
                            return $links;
                        }, 10, 2 );
