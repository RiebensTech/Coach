...
class Product_Metabox {
    ...
    public function append_product_metabox_tab($tabs){
        ...
        $new_tab = array(
            'label'    => esc_html__( 'Swatches Settings', 'swatchly-pro' ),
    ...
    public function append_product_metabox_panel(){
        ...
        <div id="swatchly_swatches_product_data" class="swatchly panel wc-metaboxes-wrapper hidden">
            ...
            <div class="toolbar">
                <button type="button" class="button swatchly_save_swatches button-primary"><?php echo esc_html__('Save Swatches', 'swatchly-pro') ?></button>
                <button type="button" class="button swatchly_reset_to_default"><?php echo esc_html__('Reset to Default', 'swatchly-pro') ?></button>
            </div>
        </div>
    ...
    public static function metabox_panel_inner_html( $product_id ){
        ...
        <div class="wc-metaboxes">
            <div class="toolbar toolbar-top swatchly_auto_convert_dropdowns_to_image_<?php echo esc_attr($auto_convert_dropdowns_to_image) ?>">
                ...
                <p class="form-fieldd">
                    ...
                    <label for="swatchly_auto_convert_dropdowns"><?php esc_html_e('Auto convert Dropdowns to Label Swatch', 'swatchly-pro') ?></label>
                </p>
                ...
                <p class="form-fieldd">
                    ...
                    <label for="swatchly_auto_convert_dropdowns_image"><?php esc_html_e('Auto convert Dropdown to Image Swatch', 'swatchly-pro') ?></label>
                </p>

                <p class="swatchly_condition swatchly_show_if_auto_convert_to_dropdown_image_1">
                    <label for="auto_convert_dropdowns_to_image_condition"><?php esc_html_e('Enable Image Type Attribute For', 'swatchly-pro') ?></label>
                    <select class="" id="auto_convert_dropdowns_to_image_condition" name="swatchly_product_meta[auto_convert_dropdowns_to_image_condition]">
                        <option value="first_attribute" <?php selected('first_attribute', $auto_convert_dropdowns_to_image_condition) ?>><?php echo esc_html__( 'The First attribute', 'swatchly-pro' ); ?></option>
                        <option value="maximum" <?php selected('maximum', $auto_convert_dropdowns_to_image_condition) ?>><?php echo esc_html__('The attribute with Maximum variations count', 'swatchly-pro') ?></option>
                        <option value="minimum" <?php selected('minimum', $auto_convert_dropdowns_to_image_condition) ?>><?php echo esc_html__('The attribute with Minimum variations count', 'swatchly-pro') ?></option>   
                    </select>
                </p>
        ...
        <?php
        $tooltip_image_size_content = esc_html__('Place the image size name here. WordPress default image sizes are: thumbnail, medium, medium_large, large and full. Custom image size also can be used. Default is: thumbnail', 'swatchly-pro'); 

        foreach( $product_attributes as $taxonomy => $terms ):
            $swatch_types = array('select', 'label', 'color', 'image', 'radio');
            ...
            $swatch_type2        = isset($meta_data[$tax_name]['swatch_type']) ? $meta_data[$tax_name]['swatch_type'] : '';
            ...
            ?>
            <div class="wc-metabox swatchly_2 swatchly_type_<?php echo esc_attr($swatch_type2); ?> swatchly_tooltip_<?php echo esc_attr($tooltip2) ?> swatchly_shape_inset_<?php echo esc_attr($enable_shape_inset2) ?>">
                <h3>
                    ...
                    <div class="fr">
                        ...
                        <!-- Swatch type 2 -->
                        <strong><?php echo esc_html__('Swatch Type', 'swatchly-pro') ?></strong>
                        <select class="swatchly_swatch_type swatchly_2" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][swatch_type]">
                            <option value=""><?php echo esc_html__('Inherit', 'swatchly-pro'); ?></option>
                            <?php
                                foreach($swatch_types as $type){
                                    ?>
                                    <option value="<?php echo esc_attr($type); ?>" <?php selected($type, $swatch_type2) ?>><?php echo esc_html(ucwords($type)); ?></option>
                                    <?php
                                }
                            ?>
                        </select>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="label" width="25%">
                                    <label for="swatchly_shape_style"><?php echo esc_html__('Shape Style', 'swatchly-pro') ?></label>
                                </td>
                                <td>
                                    <select class="swatchly_shape_style" id="swatchly_shape_style" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][shape_style]">
                                        <option value=""><?php echo esc_html__('-------', 'swatchly-pro') ?></option>
                                        <option value="squared" <?php selected('squared', $shape_style2) ?>><?php echo esc_html__('Squared', 'swatchly-pro') ?></option>
                                        <option value="rounded" <?php selected('rounded', $shape_style2) ?>><?php echo esc_html__('Rounded', 'swatchly-pro') ?></option>
                                        <option value="circle" <?php selected('circle', $shape_style2) ?>><?php echo esc_html__('Circle', 'swatchly-pro') ?></option>
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="label" width="25%">
                                    <label for="swatchly_enable_shape_inset"><?php echo esc_html__('Enable Shape Inset', 'swatchly-pro') ?></label>
                                </td>
                                <td>
                                    <select class="swatchly_enable_shape_inset" id="swatchly_enable_shape_inset" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][enable_shape_inset]">
                                        <option value=""><?php echo esc_html__('-------', 'swatchly-pro') ?></option>
                                        <option value="enable" <?php selected('enable', $enable_shape_inset2) ?>><?php echo esc_html__('Enable', 'swatchly-pro') ?></option>
                                        <option value="disable" <?php selected('disable', $enable_shape_inset2) ?>><?php echo esc_html__('Disable', 'swatchly-pro') ?></option>
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="label" width="25%">
                                    <label for="swatchly_tooltip"><?php echo esc_html__('Swatch Tooltip', 'swatchly-pro') ?></label>
                                </td>
                                <td>
                                    <select class="swatchly_tooltip" id="swatchly_tooltip" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][tooltip]">
                                        <option value=""><?php echo esc_html__('-------', 'swatchly-pro') ?></option>
                                        <option value="text" <?php selected('text', $tooltip2) ?>><?php echo esc_html__('Text', 'swatchly-pro') ?></option>
                                        <option value="image" <?php selected('image', $tooltip2) ?>><?php echo esc_html__('Image', 'swatchly-pro') ?></option>
                                        <option value="disable" <?php selected('disable', $tooltip2) ?>><?php echo esc_html__('Disable', 'swatchly-pro') ?></option>
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr class="swatchly_show_if_tooltip_text">
                                <td><label for="swatchly_tooltip_text"><?php echo esc_html__('Tooltip Text', 'swatchly-pro') ?></label></td>
                                <td>
                            ...
                            <tr class="swatchly_show_if_tooltip_image">
                                <td><?php echo esc_html__('Tooltip Image', 'swatchly-pro') ?></td>
                                <td>
                                    <div class="swatchly_media_field">
                                        ...
                                        <div>
                                            ...
                                            <button type="button" class="swatchly_upload_image button"><?php echo esc_html__( 'Upload/Add image', 'swatchly-pro' ); ?></button>
                                            <button type="button" class="swatchly_remove_image button"><?php echo esc_html__( 'Remove image', 'swatchly-pro' ); ?></button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <!-- tooltip_image_size -->
                            <tr class="swatchly_show_if_tooltip_image">
                                <td><label for="swatchly_tooltip_image_size"><?php echo esc_html__('Tooltip Image Size', 'swatchly-pro') ?></label></td>
                                <td>
                                    ...
                                    <?php  
                                        echo wp_kses_post(wc_help_tip( $tooltip_image_size_content ));
                                    ?>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="2">
                                    <?php
                                        foreach($tax_terms as $key => $term_value):
                                            if ( taxonomy_exists( $taxonomy ) ) {
                                            ...
                                            // get saved values
                                            $swatch_type        = !empty($meta_data[$tax_name]['terms'][$term_id]['swatch_type']) ? $meta_data[$tax_name]['terms'][$term_id]['swatch_type'] : $swatch_type2;
                                            ...
                                            $color_3            = isset($meta_data[$tax_name]['terms'][$term_id]['color_3']) ? $meta_data[$tax_name]['terms'][$term_id]['color_3'] : '';


                                            // Compatibility
                                            // When product is duplicated by WPML
                                            if( !empty($_GET['lang']) && get_post_type() === 'product' ){
                                                $product_meta_values = !empty($meta_data[$tax_name]['terms']) ? array_values($meta_data[$tax_name]['terms']) : array();
                                                $term_meta_value = isset($product_meta_values[$key]) ? $product_meta_values[$key] : array();

                                                if( !empty($term_meta_value['swatch_type']) ){
                                                    $swatch_type = $term_meta_value['swatch_type'];
                                                }
												
												if( !empty($term_meta_value['image']) ){
                                                    $image_id = $term_meta_value['image'];
                                                }

                                                if( !empty($term_meta_value['image_size']) ){
                                                    $image_size = $term_meta_value['image_size'];
                                                }

                                                if( !empty($term_meta_value['tooltip']) ){
                                                    $tooltip = $term_meta_value['tooltip'];
                                                }

                                                if( !empty($term_meta_value['tooltip_text']) ){
                                                    $tooltip_text = $term_meta_value['tooltip_text'];
                                                }

                                                if( !empty($term_meta_value['tooltip_image']) ){
                                                    $tooltip_image_id = $term_meta_value['tooltip_image'];
                                                }

                                                if( !empty($term_meta_value['tooltip_image_size']) ){
                                                    $tooltip_image_size = $term_meta_value['tooltip_image_size'];
                                                }

                                                if( !empty($term_meta_value['color']) ){
                                                    $color = $term_meta_value['color'];
                                                }

                                                if( !empty($term_meta_value['enable_multi_color']) ){
                                                    $enable_multi_color = $term_meta_value['enable_multi_color'];
                                                }

                                                if( !empty($term_meta_value['color_2']) ){
                                                    $color_2 = $term_meta_value['color_2'];
                                                }

                                                if( !empty($term_meta_value['color_3']) ){
                                                    $color_3 = $term_meta_value['color_3'];
                                                }
                                            }
                                            ?>
                                    <div class="wc-metabox swatchly_1 swatchly_type_<?php echo esc_attr($swatch_type); ?> swatchly_tooltip_<?php echo esc_attr($tooltip) ?> swatchly_enable_multi_color_<?php echo esc_attr($enable_multi_color) ?>">
                                        <h3>
                                            ...
                                            <div class="fr">
                                                ...
                                                <!-- Swatch type 1 -->
                                                <strong class="swatchly_d_none"><?php echo esc_html__('Swatch Type', 'swatchly-pro') ?></strong>
                                                <select class="swatchly_swatch_type swatchly_1 swatchly_d_none" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][terms][<?php echo esc_attr($term_id) ?>][swatch_type]">
                                                    <option value=""><?php echo esc_html__('Inherit', 'swatchly-pro'); ?></option>
                                                    <?php
                                                        // Removed it when making compatibility with WPML
                                                        // because term level swatch type override is not supported
                                            ...
                                            <table cellpadding="0" cellspacing="0">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <select class="swatchly_tooltip" id="swatchly_tooltip" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][terms][<?php echo esc_attr($term_id) ?>][tooltip]">
                                                                <option value=""><?php echo esc_html__('Inherit', 'swatchly-pro') ?></option>
                                                                <option value="text" <?php selected('text', $tooltip) ?>><?php echo esc_html__('Text', 'swatchly-pro') ?></option>
                                                                <option value="image" <?php selected('image', $tooltip) ?>><?php echo esc_html__('Image', 'swatchly-pro') ?></option>
                                                                <option value="disable" <?php selected('disable', $tooltip) ?>><?php echo esc_html__('Disable', 'swatchly-pro') ?></option>
                                                            </select>
                                                        </td>
                                                    </tr>
                                                    ...
                                                    <tr class="swatchly_show_if_color">
                                                        <td><?php echo esc_html__('Swatch Color', 'swatchly-pro') ?></td>
                                                    ...
                                                    <!-- enable_multi_color -->
                                                    <tr class="swatchly_show_if_color">
                                                        <td><?php echo esc_html__('Enable Multi Color', 'swatchly-pro') ?></td>
                                                    ...
                                                    <tr class="swatchly_show_if_enable_multi_color_1">
                                                        <td><?php echo esc_html__('Swatch Color 2', 'swatchly-pro') ?></td>
                                                     ...
                                                     <!-- color_3 -->
                                                     <tr class="swatchly_show_if_enable_multi_color_1">
                                                        <td><?php echo esc_html__('Swatch Color 3', 'swatchly-pro') ?></td>
                                                        <td><input type="text" class="swatchly_color_picker" name="swatchly_product_meta[<?php echo esc_attr($tax_name) ?>][terms][<?php echo esc_attr($term_id) ?>][color_3]" data-alpha-enabled="true" data-default-color="" value="<?php echo esc_attr($color_3) ?>" /></td>
                                                    </tr>
                                                    
                                                    <!-- swatchly_image -->
                                                    <tr class="swatchly_show_if_image">
                                                        <td><?php echo esc_html__('Swatch Image', 'swatchly-pro') ?></td>
                                                        <td>
                                                            <div class="swatchly_media_field">
                                                                <div class="swatchly_media_preview">
                                                                    <?php
                                                    ...
                                                    <!-- swatchly_image_size -->
                                                    <tr class="swatchly_show_if_image">
                                                        <td><label for="swatchly_image_size"><?php echo esc_html__('Swatch Image Size', 'swatchly-pro') ?></label></td>
                                                        <td>
