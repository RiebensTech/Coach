...
;( function ( $ ) {
     ...
     */
     $('#woocommerce-product-data').on('woocommerce_variations_loaded', function () {
     	var data = {
     		action: 'swatchly_pro_ajax_reload_metabox_panel',
     		product_id: swatchly_pro_params.product_id,
     		nonce: swatchly_pro_params.nonce,
     	};
    ...
    $('.swatchly_save_swatches').on('click', function(e){
    	...
    	var $message = $('.swatchly.wc-metaboxes-wrapper .woocommerce-message'),
            ...
            data = {
                action: 'swatchly_pro_ajax_save_product_meta',
                product_id: swatchly_pro_params.product_id,
		...
		$.ajax({
			...
			data: data,
			beforeSend: function(){
			    $message.html(`<p>${ swatchly_pro_params.i18n.saving }</p>`);
			},
			success: function ( response ) {
    ...
    $('.swatchly_reset_to_default').on('click', function(e){
        ...
        var $message = $('.swatchly.wc-metaboxes-wrapper .woocommerce-message'),
            $product_data = $('#woocommerce-product-data'),
            data = {
                action: 'swatchly_pro_ajax_reset_product_meta',
                product_id: swatchly_pro_params.product_id,
                nonce: swatchly_pro_params.nonce
            };
    ...
    $( document ).ready( function () {
    	...
    	if ( ! $( '.swatchly_media_field .swatchly_input' ).val() ) {
    		$( '.button.swatchly_remove_image' ).hide();
    	}
    	...
    	$( document ).on( 'click', '.button.swatchly_upload_image', function( event ) {
    		...
    		file_frame = wp.media.frames.downloadable_file = wp.media({
    			title: swatchly_pro_params.i18n.choose_an_image,
    			button: {
    				text: swatchly_pro_params.i18n.use_image
    			},
    			multiple: false
    		});
    ...
    $(document).ready(function(){
        ...
        if( parseInt(swatchly_pro_params.sp_override_global) && parseInt(swatchly_pro_params.sp_override_global) ){
            $('.csf-nav-options .csf-tab-item:first-child ul li:nth-child(1)').removeClass('swatchly_active');
        } else {
            $('.csf-nav-options .csf-tab-item:first-child ul li:nth-child(1)').addClass('swatchly_active');
        }

        if( parseInt(swatchly_pro_params.sp_override_global) ){
            $('.csf-nav-options .csf-tab-item:first-child ul li:nth-child(2)').addClass('swatchly_active');
        }
        
        if( parseInt(swatchly_pro_params.pl_override_global) ){
